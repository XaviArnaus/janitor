        ...
        for origin_file, target_info in MAP_OLD_TO_NEW.items():
            ...
            for origin_key, pairs in target_info.items():
                ...
                for pair in pairs:
                    ...
                    # What do we do here?
                    #   Actually, it is just a rename of an inner dict's key, that is behind a list.
                    #   Dictionary has support for wildcards and a get -> set would be straight forward,
                    #   But the set() is meant to apply the same value in all occurences of the wildcard,
                    #   so does not serve good here. We have to do it manually.
                    #
                    #   As usual, it is a bit overengineered (we actually know origin and target and we
                    #   could code it directly), but serves also as a propotype for a close future
                    #   "move_param" functionality.
                    #
                    # How do we do it?
                    ...
                    #   4. Execute the copy get() -> set(). Until here we could abstract it as a "copy_param"
                    ...
                    if Storage.needs_resolving(param_name=origin_key):
                        ...
                        # The get() and set() already support wildcards, but the returned values change.
                        # So we have to expect here a list of param_names, one for each resolved path.
                        origin_keys = origin_storage.resolve_wildcards(origin_key)
                        log(f"Origin key needs resolving. Got {len(origin_keys)} resolved param_names")

                    else:
                        # So there is only one path, let's emulate that we received several by making it
                        # a list of one
                        origin_keys = [origin_key]
                        log("Origin key does not need resolving.")

                    # Now we do the same with the target, but just for the parent path, because this is a rename
                    ...
                    if Storage.needs_resolving(param_name=parent_param_name):
                        # This means that we need to do something to ALL iterations of a list
                        # The get() and set() already support wildcards, but we need to set a specific value
                        # in each occurrence.
                        # So we have to expect here a list of param_names, one for each resolved path.
                        target_keys = target_storage[target_file].resolve_wildcards(parent_param_name)
                        log(f"Target key needs resolving. Got {len(origin_keys)} resolved param_names")
                    ...
                    # Now we place back the last key to each target key, so we know where the new value goes.
                    target_last_key = target_key.split(SEPARATOR)[-1]
                    target_keys = list(map(lambda x: f"{x}{SEPARATOR}{target_last_key}", target_keys))
                    log(f"Added the Last Key into {len(target_keys)} Target paths.")

                    # And before we begin anything, let's make sure we have the same amount of keys in the origin
                    #   and in the target, as we'll relate them by list index.
                    assert len(origin_keys) == len(target_keys)
                    log(f"The length of both origin and target paths are the same: {len(origin_keys)}")
                    ...
                    for index in range(0,len(origin_keys)):
                        ...
                        log(f"Will copy now from [{origin_param_name}] to [{target_param_name}]...")

                        # Execute the copy
                        value = origin_storage.get(param_name=origin_param_name)
                        target_storage[target_file].set(param_name=target_param_name, value=copy.deepcopy(value))
                        log(f"Copy of param_names in index {index} of {len(origin_keys)} is done.")
                    
                    # Verify from outside that the copy is correct
                    #   To do that we get the very original path names given without processing and compare
                    #   Yes, it has to be the same, and here lays the beauty of the verification.
                    values_in_origin_param_names = origin_storage.get(param_name=origin_key)
                    values_in_target_param_names = target_storage[target_file].get(param_name=target_key)
                    log(f"Verifying original [{origin_key}] ({len(values_in_origin_param_names)} paths) with" +
                        f"target [{target_key}] ({values_in_target_param_names} paths)")
                    for index in range(0,values_in_origin_param_names):
                        assert values_in_origin_param_names[index] == values_in_target_param_names[index]
                        log(f"Values in index {index} are correct.")
                    
                    # Reaching here means that the copy was correct. Now the deletion part of the rename.
                    log("Proceeding to delete the original param_names")
                    # There is a thought here. If the config files are actually the same (they are),
                    #   we have open the same file as origin_storage and in the target_storage[target_file].
                    #   As the internal behaviour of the Storage class, it is loaded once and written also
                    #   once and on demand, so if we change the origin storage, the target storage real file will
                    #   change below the control of the target's Storage object, and whatever new state that we
                    #   have in the origin Storage real file will be overwritten when the target's Storage object
                    #   writes the changes.
                    # Conclusion:
                    #   It would have been even easier to do all the stuff with one single config object in this 
                    #   specific case, and working only with the param_names. But we intend to make a generic
                    #   solution (remember, we're overengineering here), so:
                    #
                    #   For same files:
                    ...
                    if origin_file == target_file:
                        ...
                        for index in range(0,len(origin_keys)):
                            result = target_storage[target_file].delete(param_name=origin_keys[index])
                            log(f"Deleting [{origin_keys[index]}] from Target: {"Success" if result else "Failed"}")
                        log(f"{TerminalColor.GREEN_BRIGHT}Saving the file {target_file}{TerminalColor.END}")
                        ...
                        for index in range(0,len(origin_keys)):
                            ...
                            log(f"Deleting [{origin_keys[index]}] from Origin: {"Success" if result else "Failed"}")
                        log(f"{TerminalColor.GREEN_BRIGHT}Saving the file {target_file}{TerminalColor.END}")
                        target_storage[target_file].write_file()
                        log(f"{TerminalColor.GREEN_BRIGHT}Saving the file {origin_file}{TerminalColor.END}")
