...
class StatusPost:
    ...
    def __init__(self,
        ...
        self.visibility = visibility if visibility is not None else StatusPostVisibility.PUBLIC
        self.spoiler_text = spoiler_text
        self.language = language
        self.idempotency_key = idempotency_key
        self.content_type = content_type if content_type is not None else StatusPostContentType.PLAIN
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "scheduled_at": self.scheduled_at.timestamp() if self.scheduled_at is not None else None,
            "poll": self.poll,
    ...
    def from_dict(status_post_dict: dict) -> StatusPost:
        return StatusPost(
            ...
            status_post_dict["in_reply_to_id"] if "in_reply_to_id" in status_post_dict else None,
            ...
            StatusPostVisibility.valid_or_raise(status_post_dict["visibility"]) if "visibility" in status_post_dict else None,
            ...
            status_post_dict["idempotency_key"] if "idempotency_key" in status_post_dict else None,
            StatusPostContentType.valid_or_raise(status_post_dict["content_type"]) if "content_type" in status_post_dict else None,
            datetime.fromtimestamp(status_post_dict["scheduled_at"]) if "scheduled_at" in status_post_dict else None,
