...
def test_build_status_post_without_summary(message_without_summary: Message):
    ...
    with patch.object(Config, "get", new=mocked_config_get):
        with patch.object(formatter, "add_mention_to_message_if_direct_visibility", new=mocked_add_mention):
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ("I am a message", "@xavi", "direct", f"@xavi:\n\nI am a message"),
        ("I am a message", None, "direct", f"I am a message"),
        ("I am a message", "@xavi", "public", f"I am a message"),
    ],
)
def test_add_mention_to_message_when_is_dm(text, mention, visibility, expected_result):
    ...
    with patch.object(Config, "get", new=mocked_config_get):
        result_text = formatter.add_mention_to_message_if_direct_visibility(
            text=text
        )
    ...
    mocked_config_get.assert_has_calls(
        [
            call("mastodon.status_post.visibility"),
            call("mastodon.status_post.username_to_dm")
        ]
    )
