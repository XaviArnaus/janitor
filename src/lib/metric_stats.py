...
class MetricStats:
    ...
    def append(self, system_info: dict, exceptions: list = []) -> None:
        ...
        # First save all the metrics
        for metric_name, metric_value in system_info.items():
            if metric_name in exceptions:
                continue
            data = list(self._stack.get(metric_name, []))
            data.append(metric_value)
            self._stack.set(metric_name, data)
        # And now save the current time to get a sense of when it happened
        data = list(self._stack.get(self.TIME_METRIC_LABEL, []))
        data.append(datetime.strftime(datetime.now(timezone.utc), "%H:%M"))
        self._stack.set(self.TIME_METRIC_LABEL, data)
        # Finally write back the file
        self._stack.write_file()
    
    def get_metric(self, metric_name: str) -> list:
        return self._stack.get(metric_name, [])
    
    def get_time(self) -> list:
        return self.get_metric(self.TIME_METRIC_LABEL)
